...
public class LicensePresenter extends Presenter<LicenseView> {
    ...
    @Inject
    public LicensePresenter(final EventBusHandler bus, final LicenseClient client) {
        setBus(bus);
    ...
    @Subscribe
    public void setLicenseClient(@NotNull final LicenseClient licenseClient) {
    ...
    private OfficeLicense getLicense() {
        if (license == null)
            license = licenseClient.getLicense();
