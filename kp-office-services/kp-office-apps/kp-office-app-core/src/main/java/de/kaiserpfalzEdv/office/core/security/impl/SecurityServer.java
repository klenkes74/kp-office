...
import de.kaiserpfalzEdv.office.core.security.commands.SecurityCommand;
...
public class SecurityServer implements MessageListener {
    private static final Logger LOG = LoggerFactory.getLogger(SecurityServer.class);

    private RabbitTemplate      sender;
    private SecurityApplication service;


    @Inject
    public SecurityServer(final RabbitTemplate sender, final SecurityApplication service) {
        this.sender = sender;
    ...
    public void onMessage(Message message) {
        SecurityCommand command = (SecurityCommand) SerializationUtils.deserialize(message.getBody());
