...
public class LicenseBuilder implements Builder<OfficeLicense> {
    ...
    private void validate() {
        ...
        if (id == null) failures.add("No valid license id is given!");
        if (issued == null) failures.add("No valid issue date is given!");
        if (isBlank(issuer)) failures.add("No issuer for this license is given!");
        if (isBlank(licensee)) failures.add("No licensee for this license is given!");
        if (starts == null) failures.add("No start date for license validity!");
        if (expires == null) failures.add("No expiry date for license!");
        if (isBlank(software)) failures.add("No software name for this license is given!");
        if (versionRange == null) failures.add("No validity range for the version for this license!");
