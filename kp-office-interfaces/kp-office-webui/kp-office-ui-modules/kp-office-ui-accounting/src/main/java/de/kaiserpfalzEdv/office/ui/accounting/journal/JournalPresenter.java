 ...
 */
@VaadinSessionScope
@VaadinPresenter(viewName = JournalView.NAME)
public class JournalPresenter extends Presenter<JournalView> {
    private static final Logger LOG = LoggerFactory.getLogger(JournalPresenter.class);

    @Inject
    private JournalTableBuilder dataLoader;
    ...
    public void openJournal(Event<OpenJournalEvent> event) {
        LOG.trace("{} received: {}", event);

        if (open) {
            LOG.info("Closing journal tab!");
            RemoveMainTabEvent closeTab = new ContentTabBuilder()
                    .withId(journalTabId)
                    .removeMainTabEvent();
            getEventBus().publish(EventScope.SESSION, this, closeTab);
        } else {
            LOG.info("Opening journal tab!");
