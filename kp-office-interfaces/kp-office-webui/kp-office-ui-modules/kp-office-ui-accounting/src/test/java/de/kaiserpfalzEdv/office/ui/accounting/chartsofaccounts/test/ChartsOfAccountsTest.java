...
package de.kaiserpfalzEdv.office.ui.accounting.chartsofaccounts.test;
...
public class ChartsOfAccountsTest extends SpringTestNGBase {
    ...
    public void testAccounts() {
        ...
        assertEquals(accounts.getName(), "Internal Chart Of Accounts", "Wrong name of chart of accounts!");
    ...
    public void testMapping(final Account account, final String d03Number, final String d04Number) {
        logMethod("check-mapping", "Check mapping from Account '" + account.getName() + "' to '" + d03Number + "' (D03) and '" + d04Number + "' (D04) ...");

        assertEquals(
                d03Mapping.renumber(account)
                          .getNumber(), d03Number, "Renumbering " + account.getId() + " for D03 failed!"
        );
        assertEquals(
                d04Mapping.renumber(account)
                          .getNumber(), d04Number, "Renumbering " + account.getId() + " for D04 failed!"
        );
    }
    ...
    public void testBookableAccounts(final AccountMapping mapping, final String number, final boolean bookable) {
        ...
        assertEquals(mapping.isBookable(number), bookable, "In mapping '" + mapping.getName() + "' the account '" + number + "' should be " + (bookable ? "" : "not ") + "bookable!");
    }
