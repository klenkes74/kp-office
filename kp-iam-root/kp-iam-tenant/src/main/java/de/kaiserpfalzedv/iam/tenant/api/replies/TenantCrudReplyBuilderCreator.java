...
public class TenantCrudReplyBuilderCreator implements CrudReplyBuilderCreator<Tenant> {
    @Override
    public TenantCreateReply create(UUID source, UUID commandId, UUID replyId, Tenant data) {
        return new TenantCreateReply(source, commandId, replyId, data);
    }

    @Override
    public TenantRetrieveReply retrieve(UUID source, UUID commandId, UUID replyId, PagedListable<Tenant> data) {
        return new TenantRetrieveReply(source, commandId, replyId, data);
    }

    @Override
    public TenantUpdateReply update(UUID source, UUID commandId, UUID replyId, Tenant data) {
        return new TenantUpdateReply(source, commandId, replyId, data);
    }

    @Override
    public TenantDeleteReply delete(UUID source, UUID commandId, UUID replyId, UUID dataId) {
