...
import de.kaiserpfalzedv.commons.api.data.*;
 ...
 * @since 1.0.0
 */
public class JPAEntitlementRepository implements Repository<JPAEntitlement> {
    private static final Logger LOG = LoggerFactory.getLogger(JPAEntitlementRepository.class);
    ...
    public PagedListable<JPAEntitlement> retrieve(@NotNull final Pageable page) {
        List<JPAEntitlement> data = em
                .createNamedQuery("fetch-all", JPAEntitlement.class)
        ...
        if (totalCount == 0) {
            ...
            totalCount = em.createNamedQuery("fetch-all.count", Long.class)
                    .getSingleResult();
