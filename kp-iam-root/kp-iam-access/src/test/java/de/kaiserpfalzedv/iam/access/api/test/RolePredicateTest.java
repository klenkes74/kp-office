...
public class RolePredicateTest {
    private static final Logger LOG = LoggerFactory.getLogger(RolePredicateTest.class);

    private static final UUID id = UUID.fromString("44444444-4444-4444-4444-444444444444");

    private PredicateToQueryParser<Role> queryParser;
    private PredicateToParameterParser<Role> parameterParser;

    private Predicate<Role> cut;
    ...
    public void shouldGenerateValidQuery() {
        ...
        assertEquals(
                "((id=:id_EQUALS) and (displayName<>:displayName_NOT_EQUALS)) and (fullName<>:fullName_NOT_EQUALS)",
                result
        );
    }
    ...
    public void setUp() {
        ...
        cut = RolePredicate.id().isEqualTo(id)
                           .and(RolePredicate.displayName().isNotEqualTo("abc"))
                           .and(RolePredicate.fullName().isNotEqualTo("def"));
    }
