...
import de.kaiserpfalzEdv.piracc.organization.identity.list.IdentityListViewImpl;
...
@Named
@UIScope
public class IdentityEditorPresenter extends BaseEditorPresenterImpl<Identity, IdentityEditorView> {
    private static final Logger LOG = LoggerFactory.getLogger(IdentityEditorPresenter.class);

    @Inject
    public IdentityEditorPresenter(
            final EntityService<Identity> service,
    ...
    public void navigateToTarget(Object source) {
        ...
        navigateTo(IdentityListViewImpl.VIEW_NAME);
    }


    @Subscribe
    public void loadData(LoadIdentityEvent event) {
        LOG.debug("Loading data: {}", event);

        load(event.getPayload());

        if (Objects.equals(getData().getId(), event.getPayload()))
            navigateTo(IdentityEditorViewImpl.VIEW_NAME);
    }
}
