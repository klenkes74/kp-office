...
package de.kaiserpfalzEdv.piracc.tech.account.list;
...
@MenuEntry(
        name = AccountListViewImpl.VIEW_NAME,
        i18nKey = AccountListViewImpl.VIEW_NAME,
        access = {"dsb", "board", "office"},
        order = 12,
        separator = false
)
@SpringView(name = AccountListViewImpl.VIEW_NAME)
public class AccountListViewImpl extends BaseListViewImpl<Account, AccountListView> implements AccountListView, View {
    public static final  String VIEW_NAME        = "accounts.list";
    private static final long   serialVersionUID = -7034688590121887607L;


    @Inject
    public AccountListViewImpl(
            final AccountListPresenterImpl presenter,
            final DataContainerProvider dataContainerProvider
    ) {
        super(presenter, dataContainerProvider, Account.class);
    }


    @Override
    protected ArrayList<TableDescription> initializeFields() {
        ArrayList<TableDescription> result = new ArrayList<>(10);

        result.add(new TableDescription("id", 5).setI18nHeadingKey("base.id"));

        result.add(new TableDescription("system.name", 15).setI18nHeadingKey("systems.name"));
        result.add(new TableDescription("account", 15).setI18nHeadingKey("accounts.account"));

        result.add(new TableDescription("from", 10).setI18nHeadingKey("base.from"));
        result.add(new TableDescription("accountCreated", 10).setI18nHeadingKey("accounts.accountCreated"));
        result.add(new TableDescription("till", 10).setI18nHeadingKey("base.till"));
        result.add(new TableDescription("accountRemoved", 10).setI18nHeadingKey("accounts.accountRemoved"));
