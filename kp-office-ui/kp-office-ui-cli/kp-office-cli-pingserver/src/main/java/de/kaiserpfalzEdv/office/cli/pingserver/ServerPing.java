...
import de.kaiserpfalzEdv.office.core.license.OfficeLicence;
import de.kaiserpfalzEdv.office.core.license.impl.NullLincenceImpl;
...
public class ServerPing implements CliModule {
    ...
    public ServerPing(final LicenceClient client, final EventBusHandler bus) {
        ...
        LOG.trace("{} license client: {}", this.client);
    ...
    public void execute(ExecutionCommand event) {
        ...
        if ("ping".equalsIgnoreCase(action)) {
            try {
                ...
                LOG.info("Server responded to license request: {}", licence);

                System.out.println("Server answered to license request.");
        ...
        if ("help".equalsIgnoreCase(action)) {
            System.out.println(String.format("%-14s %s", "ping", "Retrieves the license of the server to check availability."));
        }
    }
